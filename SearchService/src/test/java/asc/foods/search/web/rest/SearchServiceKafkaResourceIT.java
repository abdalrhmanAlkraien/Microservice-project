...
import asc.foods.search.config.KafkaProperties;
...
class SearchServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        SearchServiceKafkaResource kafkaResource = new SearchServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/search-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/search-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
