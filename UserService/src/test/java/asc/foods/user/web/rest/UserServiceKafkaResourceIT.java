...
import asc.foods.user.config.KafkaProperties;
...
class UserServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        UserServiceKafkaResource kafkaResource = new UserServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/user-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/user-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
