...
import asc.foods.gateway.config.KafkaProperties;
...
class GatewayServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        GatewayServiceKafkaResource kafkaResource = new GatewayServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gateway-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/gateway-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
